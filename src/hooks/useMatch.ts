...
export function useMatch() {
  ...
  const selectBowler = useCallback((bowlerPlayerIndex: number) => {
    setState((s) => {
      ...
      if (bowlerIdx === -1) {
        bowlers.push({
          name: team.players[bowlerPlayerIndex],
          playerIndex: bowlerPlayerIndex,
          oversBowled: 0,
          ballsBowled: 0,
          runsConceded: 0,
          wickets: 0,
      ...
      setState((s) => {
        ...
        switch (type) {
          ...
          case "6":
            ...
            break;
          case "wide":
            // Wide: 1 for the wide + extra runs if provided
            const wideExtraRuns = howOut ? parseInt(howOut, 10) : 0;
            totalRuns = 1 + wideExtraRuns;
            extras = totalRuns;
            isLegal = false;
            label = wideExtraRuns > 0 ? `WD+${wideExtraRuns}` : "WD";
            break;
          case "noball":
            // No-ball: 1 for the no-ball + extra runs if provided
            const nbExtraRuns = howOut ? parseInt(howOut, 10) : 0;
            totalRuns = 1 + nbExtraRuns;
            batsmanRuns = nbExtraRuns; // Batsman gets credit for runs off no-ball
            extras = 1; // Only the no-ball itself is an extra
            isLegal = false;
            label = nbExtraRuns > 0 ? `NB+${nbExtraRuns}` : "NB";
            break;
          case "wicket":
            isWicket = true;
            label = "W";
        ...
        // For no-ball, batsman gets credit for runs
        if (type === "noball" && batsmanRuns > 0) {
          striker.runs += batsmanRuns;
          if (batsmanRuns === 4) striker.fours += 1;
          if (batsmanRuns === 6) striker.sixes += 1;
        }
        ...
        const endInnings = (
          ...
          if (innIdx === 0) {
            return {
              ...s,
              innings: newInnings,
              phase: "innings-break" as MatchPhase,
              target: currentInn.score + 1,
              lastCompletedOver: lastOver,
            };
          }
          const firstScore = s.innings[0]!.score;
          let winner: number | null;
          if (currentInn.score > firstScore)
            winner = currentInn.battingTeamIndex;
          else if (currentInn.score < firstScore)
            winner = 1 - currentInn.battingTeamIndex;
          else winner = -1;
          return {
            ...s,
            innings: newInnings,
        ...
        if (!isWicket && !isLegal && type === "noball") {
          const nbExtraRuns = howOut ? parseInt(howOut, 10) : 0;
          if (nbExtraRuns % 2 === 1) {
            const temp = inn.strikerIdx;
            inn.strikerIdx = inn.nonStrikerIdx;
        ...
        const legalBallsInOver = inn.currentOver.filter(
        ...
        if (legalBallsInOver >= 6) {
          ...
          return {
            ...s,
            innings: newInnings,
            phase: "new-bowler" as MatchPhase,
            lastCompletedOver: lastOver,
          };
        }
