...
export default function LiveMatch({
  ...
  const [extraType, setExtraType] = useState<"wide" | "noball" | null>(null);
  const [extraRuns, setExtraRuns] = useState<number | null>(null);
  ...
  const handleScoringAction = (action: ScoringAction) => {
    switch (action.type) {
      case "DOT":
        ...
        break;
      case "RUNS":
        onScore(String(action.runs) as "1" | "2" | "3");
        break;
      case "FOUR":
      ...
      case "SIX":
        ...
        break;
      case "WIDE":
        // Show dialog to select extra runs
        setExtraType("wide");
        break;
      case "NOBALL":
        // Show dialog to select extra runs
        ...
        break;
      case "BYE":
        onScore(String(action.runs) as "1" | "2" | "3" | "4" | "6");
        break;
      case "LEGBYE":
        onScore(String(action.runs) as "1" | "2" | "3" | "4" | "6");
        break;
      case "WICKET":
        ...
        onScore("wicket", action.howOut);
        break;
    }
  };

  const confirmExtraRuns = () => {
    if (extraType && extraRuns !== null) {
      if (extraRuns > 0) {
        onScore(extraType, String(extraRuns));
      } else {
        onScore(extraType);
      }
      setExtraType(null);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col max-w-lg mx-auto">
              ...
              <span
                ...
                className={`w-8 h-8 flex items-center justify-center rounded-full text-xs font-bold ${
                  b.isWicket
                    ...
                    : b.runs >= 4
                      ? "bg-boundary text-boundary-foreground"
                      : b.isWide || b.isNoBall
                        ? "bg-extras-color text-extras-foreground"
                        : b.runs === 0
      ...
      {/* Extra Runs Dialog for Wide/No-ball */}
      <Dialog
        open={extraType !== null}
        onOpenChange={() => {
          setExtraType(null);
        ...
        <DialogContent className="bg-card border-border max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-foreground">
              {extraType === "wide" ? "Wide" : "No Ball"} + Extra Runs?
            </DialogTitle>
          </DialogHeader>
          <div className="mt-2">
                ...
                <button
                  ...
                  className={`p-4 rounded-lg font-bold text-lg transition-all ${
                    extraRuns === runs
                      ...
                      : "bg-secondary text-foreground border-2 border-transparent hover:border-primary/50"
                  }`}
                >
                  {runs === 0 ? "No Extra" : `+${runs}`}
                </button>
              ))}
            </div>
          </div>
          <Button
            onClick={confirmExtraRuns}
            disabled={extraRuns === null}
